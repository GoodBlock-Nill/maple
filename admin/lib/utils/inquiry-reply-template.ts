...
export const INQUIRY_REPLY_PLACEHOLDERS: readonly InquiryPlaceholder[] = [
  ...
  { token: '{{문의번호}}', description: '문의 ID 앞 8자리(대문자). 사용자가 대조하는 접수번호' },
...
/**
 * 접수번호 — 문의 ID 앞 8자리(대문자).
 *
 * UUID 36자를 그대로 적으면 사용자가 옮겨 적지 못하고, 답변 문장도 읽히지 않는다.
 * 앞 8자리는 관리자 목록에서 문의를 찾기에 충분하고(주소창의 id 앞부분과 같다),
 * 공개돼도 다른 문의를 추측할 수 있는 값이 아니다.
 */
export function inquiryNumber(id: string): string {
