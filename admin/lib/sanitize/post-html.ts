...
import { parseVideoToken, toVideoToken } from '@/lib/sanitize/video-embed'
import { requireEnv } from '@/lib/supabase/env'

/**
 * 뉴스 본문(HTML) 정제기 — 사용자 사이트 `lib/sanitize/post-html.ts` 의 사본.
 *
 * **왜 사본인가.** 관리자가 저장한 값을 사용자 사이트가 그대로 렌더한다. 허용
 * 목록이 갈라지면 두 방향으로 깨진다 — 관리자가 넣었는데 사용자 화면에서 사라지거나
 * (관리자 쪽이 더 관대), 저장할 수 없는데 화면은 허용하거나. 두 파일은 항상 함께
 * 고친다. 지금은 워크스페이스 공유 패키지가 없어 사본이 유일한 방법이다.
 *
 * **`sanitize-html` 을 고른 이유.** `isomorphic-dompurify` 는 서버에서 돌기 위해
 * `jsdom` 을 통째로 끌고 온다. `sanitize-html` 은 `htmlparser2` 기반이라 DOM 이
 * 필요 없고, 무엇보다 **속성값 단위 검사**(`transformTags`)를 선언적으로 쓸 수 있다 —
 * img 는 우리 스토리지 공개 URL 접두사로 시작할 때만, a 는 http(s) 일 때만 살린다.
 *
 * **신뢰 경계.** 이 함수의 출력만이 DB 에 저장된다. 에디터를 우회한 직접 POST 도
 * 같은 규칙을 받는다.
 */
...
export function postImagePublicUrlPrefix(supabaseUrl: string): string {
  return `${supabaseUrl.replace(/\/+$/u, '')}/storage/v1/object/public/${POST_IMAGE_BUCKET}/`
}
...
/** 저장 직전에 반드시 통과시킨다. 출력만이 DB 와 사용자 화면이 신뢰하는 값이다. */
