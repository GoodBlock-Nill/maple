...
import { socialSignInFormAction } from '@/lib/actions/auth-actions'
...
type SocialSignInButtonsProps = {
  ...
  nextPath: string
  /** `?error=` 로 넘어온 안내(권한 없음 · 세션 만료 …). 없으면 빈 문자열. */
  initialError?: string
}

/**
 * 간편로그인 버튼 묶음 — 관리자 로그인의 기본 수단.
 *
 * 세 버튼을 폼 하나에 담고, 눌린 버튼의 `name="provider"` 값으로 어떤 제공자를
 * 골랐는지 전달한다. 버튼마다 폼을 따로 두면 오류 표시 자리를 셋으로 나눠야 해서
 * 화면이 흔들린다. 자바스크립트가 없어도 제출은 그대로 동작한다(진행 표시만 빠진다).
 */
export function SocialSignInButtons({ nextPath, initialError }: SocialSignInButtonsProps) {
  const [state, formAction, isPending] = useActionState(socialSignInFormAction, EMPTY_FORM_STATE)
  const [clicked, setClicked] = useState<SocialProvider | null>(null)

  /* 액션이 아직 아무것도 돌려주지 않았을 때만 URL 로 받은 안내를 보여 준다.
      ...
      <form action={formAction} className="flex flex-col gap-2.5">
        ...
        {SOCIAL_PROVIDER_STYLES.map(({ provider, label, surfaceClass, icon }) => (
          <button
            ...
            className={cn(
              'rounded-pill flex h-[46px] w-full items-center justify-center gap-2 text-[15px] font-semibold',
