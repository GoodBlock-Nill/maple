...
import { POST_IMAGE_ACCEPT } from '@/components/editor/post-image'
import { CONTENT_CLASS } from '@/components/editor/typography'
 ...
 * 툴바의 `position: sticky` 가 통째로 무력화된다. 모서리는 툴바가 `rounded-t-card`
 * 로 맞춘다.
 */
...
export function PostEditor({ name, label, defaultValue = '', hint, error }: PostEditorProps) {
  ...
  const editor = useEditor({
    ...
    editorProps: {
      /* ProseMirror 루트에 직접 붙는다. 래퍼에 붙이면 실제 입력 요소(role=textbox)
