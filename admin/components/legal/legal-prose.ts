/**
 * DB 에서 온 정책 HTML 의 화면 서식 — 사용자 사이트
 * `components/policy/policy-prose.ts` 의 **사본**이다.
 *
 * 관리자 미리보기와 독자 화면이 한 글자도 다르면 안 된다. 워크스페이스 공유
 * 패키지가 없어 파일을 복사해 두었고, `tests/unit/legal-prose.test.ts` 가 원본과
 * 사본이 어긋나는 순간 깨진다. 색 토큰은 관리자 `app/globals.css` 가 사용자 사이트
 * 값을 그대로 정의하므로(`.legal-preview`) 클래스 문자열까지 같게 유지할 수 있다.
 *
