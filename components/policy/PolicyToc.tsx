  ...
  return (
    <>
      <details className="border-line-soft rounded-panel bg-surface group border p-5 lg:hidden">
        <summary className="text-ink flex cursor-pointer list-none items-center justify-between text-[16px] font-semibold">
          목차
    ...
    <ol className={cn('flex flex-col gap-2.5', className)}>
      {sections.map((section) => (
        <li key={section.id}>
          <Link
            href={`#${section.id}`}
            className="text-ink-muted hover:text-ink rounded-pill text-[15px] transition-colors"
          >
