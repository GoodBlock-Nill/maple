...
import { toVideoToken } from '@/lib/utils/video-embed'

import type { VideoEmbed } from '@/lib/utils/video-embed'

/**
 * 영상 임베드 노드.
 *
 * `@tiptap/extension-youtube` 를 쓰지 않는 이유: 그 확장은 본문에 `<iframe>` 을
 * 직접 남기는데, 우리는 저장 형식을 자리표시자(`<div data-video="youtube:<id>">`)로
 * 정했다(`lib/sanitize/post-html.ts` 참고). 자리표시자를 쓰면 정제기의 허용 목록에
