 ...
 * 굵게를 한 번 누를 때마다 글이 등록된다. 토글 상태는 `aria-pressed` 로 알린다
 * (스크린 리더에서 "굵게, 누름"으로 읽힌다).
 *
 ...
 * 툴바는 에디터 상단에 붙어 따라온다.
 *
 * `top-0` 이 아니라 104px 인 이유: 사이트 헤더가 `sticky top-5` 로 떠 있고 높이가
 * 72~76px 이라(`SiteHeader`) 0에 붙이면 툴바가 헤더 **아래로 파고들어** 서식 버튼이
 * 가려진다. 헤더 바닥(20+76=96)에 여백 8을 더한 값이다.
 */
const TOOLBAR_CLASS =
  'border-line-soft bg-surface sticky top-[104px] z-10 rounded-t-[12px] border-b'
