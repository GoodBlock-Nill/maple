 ...
 *
 * 지금 연락처(`care@gjstory.com`)는 처음부터 ASCII 도메인이라 이 변환이 사실상
 * 통과(no-op)한다. 예전 연락처(`contact@글자월드.co.kr`)는 한글 도메인이었다 —
 * 시안은 한글 그대로 보여 주지만 `mailto:` href 에 한글을 그대로 넣으면 일부 메일
 * 클라이언트가 주소를 열지 못해, 화면에는 유니코드(IDN), href 에는 ASCII(Punycode)
 * 를 쓰는 이 변환이 필요했다. 관리자가 `site_settings.contact_email` 에 다시 IDN
 * 도메인을 입력할 수 있으므로 로직은 그대로 남겨 둔다.
 *
...
/** 화면 표기용. 예: `hello@xn--...` (Punycode) → `hello@한글도메인.kr` */
export function toDisplayEmail(email: string): string {
  return convert(email, domainToUnicode)
}

/** `mailto:` 링크용. 예: `hello@한글도메인.kr` → `hello@xn--...` (Punycode) */
