...
이틀 동안(09-10 · 09-11) 1:1 문의는 **폼 하나에서 운영 도구로** 바뀌었습니다. 첫날은 접수가 실제로 되게 만드는 일(첨부 실패 수정 → 카테고리·프리필 → 영상 첨부)이었고, 둘째 날은 들어온 문의를 **여러 운영자가 겹치지 않게 처리**하는 일(필수 규칙 → 답변 템플릿 → 회원 연결 → 배정·잠금·충돌·메모·접수번호)이었고, 09-11 오후에는 협업 기능이 안정된 뒤 **고객지원 화면 전체를 새 시안(v2)으로 다시 그렸습니다**. 사흘 뒤 09-14 에는 **1:1 문의 하나였던 창구가 셋으로 늘었습니다** — 버그제보·불법이용제보가 같은 폼·같은 목록·같은 권한 위에 **종류(kind)** 축 하나로 추가됐습니다(§3). 같은 날 오후에는 **처리 중인 문의에 한해 사용자가 답장할 수 있는 대화 스레드**가 열리고, 쿠폰 관리자 화면이 걷혔습니다(§2). 09-14 저녁에는 **첨부 규칙이 형식과 무관하게 통일**됐습니다 — 이미지·PDF·영상 가릴 것 없이 최대 5개 · 합계 200MB, 모든 첨부가 브라우저에서 버킷으로 직접 올라갑니다(§1). 하루 뒤 09-15 에는 **회원 답장 창이 3건에서 1건으로** 좁아졌습니다 — 운영자 답변 하나에 답장 하나입니다(§0).

---

## 0. 2026-09-15 — 회원 답장 창 3건 → 1건

오너 결정(2026-09-15)으로 회원 답장이 **운영자 답변 하나당 1건**이 됐습니다(그전에는 마지막 운영자 답변 이후 3건까지 이어 쓸 수 있었습니다). 창이 다시 열리는 셈법은 그대로입니다 — 운영자가 한 번 더 답하면 그 뒤로 다시 1건을 보낼 수 있습니다. 나머지 규칙(본인 · 취소 아님 · 처리 중 · 운영자 답변 있음 · 내용·첨부 상한)과 검사 순서는 바뀌지 않았습니다. 자세한 설계·동작은 `docs/admin/INQUIRY-GUIDE.md` §2.8 을 보세요 — 이 절은 **무엇이 바뀌었는지**만 요약합니다.
...
- **클라이언트** — `lib/constants/inquiry-thread.ts`(상수·문구) · `lib/utils/inquiry-thread.ts`(주석만).
- **관리자** — 코드 변경 없음. 답변 폼의 "처리 중: 회원이 이 문의에 답장할 수 있습니다" 안내는 건수를 적지 않아 그대로 둡니다.
- **테스트** — 클라이언트 단위 **1391건 전체 통과**(`inquiry-thread.test.ts` 에 "새 답변 뒤 답장 한 건이면 다시 닫힌다" 한 건 추가) · e2e `support-inquiries.spec.ts` **13건 전체 통과**(회원 답장 시나리오가 한 건을 보낸 뒤 창이 닫히는 것까지 확인). 실서버 RPC 도 서비스 롤로 한 번 확인했습니다 — 1건째 성공 · 2건째 `too_many` · 운영자 답변 뒤 다시 성공(확인용 데이터는 모두 삭제).
