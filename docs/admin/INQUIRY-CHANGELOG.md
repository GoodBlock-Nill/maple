# 고객지원(1:1 문의 · 버그제보 · 불법이용제보) — 2026-09-10 · 09-11 · 09-14 · 09-15 변경 사항 정리

기준 커밋 `6fd50de`(2026-09-14) · 대상 기간 **2026-09-10 ~ 2026-09-11 · 2026-09-14 · 2026-09-15** · 현재 상태 문서 `docs/admin/INQUIRY-GUIDE.md`
...
이틀 동안(09-10 · 09-11) 1:1 문의는 **폼 하나에서 운영 도구로** 바뀌었습니다. 첫날은 접수가 실제로 되게 만드는 일(첨부 실패 수정 → 카테고리·프리필 → 영상 첨부)이었고, 둘째 날은 들어온 문의를 **여러 운영자가 겹치지 않게 처리**하는 일(필수 규칙 → 답변 템플릿 → 회원 연결 → 배정·잠금·충돌·메모·접수번호)이었고, 09-11 오후에는 협업 기능이 안정된 뒤 **고객지원 화면 전체를 새 시안(v2)으로 다시 그렸습니다**. 사흘 뒤 09-14 에는 **1:1 문의 하나였던 창구가 셋으로 늘었습니다** — 버그제보·불법이용제보가 같은 폼·같은 목록·같은 권한 위에 **종류(kind)** 축 하나로 추가됐습니다(§3). 같은 날 오후에는 **처리 중인 문의에 한해 사용자가 답장할 수 있는 대화 스레드**가 열리고, 쿠폰 관리자 화면이 걷혔습니다(§2). 09-14 저녁에는 **첨부 규칙이 형식과 무관하게 통일**됐습니다 — 이미지·PDF·영상 가릴 것 없이 최대 5개 · 합계 200MB, 모든 첨부가 브라우저에서 버킷으로 직접 올라갑니다(§1). 하루 뒤 09-15 에는 **회원 답장 창이 3건에서 1건으로** 좁아졌고 — 운영자 답변 하나에 답장 하나입니다 — 그 규칙이 연타를 대신 막게 되면서 **답장의 30초 쿨다운도 걷혔습니다**(접수의 30초는 유지, §0).

---

## 0. 2026-09-15 — 회원 답장 창 3건 → 1건 · 답장 쿨다운 제거

오너 결정(2026-09-15)으로 회원 답장이 **운영자 답변 하나당 1건**이 됐습니다(그전에는 마지막 운영자 답변 이후 3건까지 이어 쓸 수 있었습니다). 창이 다시 열리는 셈법은 그대로입니다 — 운영자가 한 번 더 답하면 그 뒤로 다시 1건을 보낼 수 있습니다. 같은 날, 창이 1건이 되면서 의미가 없어진 **답장의 30초 쿨다운을 걷어냈습니다** — 접수 쪽 30초는 그대로입니다. 나머지 규칙(본인 · 취소 아님 · 처리 중 · 운영자 답변 있음 · 내용·첨부 상한)은 바뀌지 않았습니다. 자세한 설계·동작은 `docs/admin/INQUIRY-GUIDE.md` §2.8 을 보세요 — 이 절은 **무엇이 바뀌었는지**만 요약합니다.

| 날짜 · 시각 | 커밋 | 영역 | 변경 요약 | 마이그레이션 |
| ----------- | ---- | ---- | --------- | ------------- |
| 09-15 | (이번 커밋) | DB · 클라이언트 | **답장 창 3건 → 1건** — RPC `add_inquiry_user_reply()` 의 `too_many` 문턱(`>= 3` → `>= 1`) · 상수 `INQUIRY_USER_REPLY_WINDOW` 를 1 로 · 안내 문구를 `운영자 답변을 기다려 주세요. 운영자 답변 하나에 답장은 1건만 보낼 수 있습니다.` 로 교체 | `20260915000100` |
| 09-15 | (이번 커밋) | 클라이언트 | 답장 30초 쿨다운 제거 — 1건 규칙이 대신 막음, 접수 30초는 유지 | — |

### 무엇이 바뀌었나

- **DB 는 함수 재정의 하나뿐입니다** — `20260915000100_inquiry_user_reply_window_1.sql` 이 `add_inquiry_user_reply()` 를 다시 만들면서 문턱 한 줄(`recent_reply_count >= 3` → `>= 1`)만 바꿉니다. 반환 코드 집합(`not_owner` · `cancelled` · `not_in_progress` · `no_operator_reply` · `too_many` · `invalid`)과 검사 순서는 그대로입니다 — 순서가 곧 안내 문구의 우선순위라 흔들면 화면 판정과 어긋납니다.
- **이미 쌓인 답장은 건드리지 않습니다** — 백필이 없어 지난 대화는 그대로 남고, 다음 답장부터 새 문턱이 걸립니다.
- **클라이언트는 상수 하나입니다** — `lib/constants/inquiry-thread.ts` 의 `INQUIRY_USER_REPLY_WINDOW` 가 1 이 되고, 화면 판정 `canUserReply()`(`lib/utils/inquiry-thread.ts`)는 그 상수를 읽으므로 로직 변경이 없습니다. "3건까지"가 어색해진 안내 문구만 새로 썼습니다 — `운영자 답변을 기다려 주세요. 운영자 답변 하나에 답장은 1건만 보낼 수 있습니다.`
- **운영자에게 보이는 변화** — 회원이 한 번 답장하면 그 문의의 답장 폼이 닫힙니다. 추가로 물을 것이 있으면 **한 번에 모아 묻는 편**이 좋습니다. 운영자가 다시 답하면 회원이 한 번 더 답할 수 있습니다.
- **답장의 30초 쿨다운을 걷어냈습니다** — 창이 1건이 되면서 연타할 자리 자체가 없어졌습니다(RPC 의 `too_many` 가 두 번째 답장을 거절합니다). `replyToInquiry` 에서 `remainingCooldown` 검사를 빼고, 접수·답장 시각을 함께 보던 `getLatestInquiryWriteAt()` 도 지웠습니다. **접수의 30초는 그대로**이고, 이제 접수 시각끼리만 봅니다(`getLatestInquiryAt`) — 답장했다고 새 문의 접수가 막히던 경계가 사라집니다.

### 영향 범위

- **DB — 마이그레이션 있음**: `supabase/migrations/20260915000100_inquiry_user_reply_window_1.sql`(`add_inquiry_user_reply()` 재정의, 다른 객체는 그대로). 운영 DB 에 적용 완료(2026-09-15).
- **클라이언트** — `lib/constants/inquiry-thread.ts`(상수·문구) · `lib/utils/inquiry-thread.ts`(주석만) · `lib/actions/inquiry-reply-actions.ts`(쿨다운 검사·임포트 제거) · `lib/actions/inquiry-cooldown.ts`(`getLatestInquiryWriteAt` · `getLatestUserReplyAt` 삭제, `getLatestInquiryAt` 만 남김).
- **관리자** — 코드 변경 없음. 답변 폼의 "처리 중: 회원이 이 문의에 답장할 수 있습니다" 안내는 건수를 적지 않아 그대로 둡니다.
- **테스트** — 클라이언트 단위 **1395건 전체 통과**(`inquiry-thread.test.ts` 에 "새 답변 뒤 답장 한 건이면 다시 닫힌다" 한 건 추가 · 쿨다운 제거와 함께 `tests/unit/actions/inquiry-reply-actions.test.ts` 신규 4건 — 쿨다운 조회가 사라졌음을 "테이블 조회 0건"으로 고정합니다) · e2e `support-inquiries.spec.ts` **13건 전체 통과**(47초, 회원 답장 시나리오가 한 건을 보낸 뒤 창이 닫히는 것까지 확인). 답장 시나리오에는 기다리는 구간이 없었고(운영자 답변을 서비스 롤로 심습니다), 31초를 기다리는 것은 **접수** 두 건을 내는 창구 시나리오 하나뿐입니다 — 접수 쿨다운은 그대로라 그 대기도 그대로 둡니다. 실서버 RPC 도 서비스 롤로 한 번 확인했습니다 — 1건째 성공 · 2건째 `too_many` · 운영자 답변 뒤 다시 성공(확인용 데이터는 모두 삭제).

---

## 1. 2026-09-14 (3) — 첨부 규칙 통일: 형식 무관 5개 · 200MB
...
## 2. 2026-09-14 (2) — 회원 답장 스레드 · 답변 기본 상태 처리 중 · 답변 템플릿 문구 정정 · 쿠폰 관리자 화면 제거
...
| 09-14 | `c7c624a` | DB · 관리자 | **쿠폰 관리자 화면 제거**(테이블은 보존, 데이터는 유지) · **문의 대화 스레드 DB** — `inquiry_replies.attachments` · `inquiries.user_replied_at` · 트리거 `touch_inquiry_on_reply()` · RPC `add_inquiry_user_reply()`(처리 중 · 운영자 답변 후 · 연속 3건) · 템플릿 문구를 다시 "답장으로" 되돌림(§2 아래) | `20260914000400` |
...
- 회원 답장은 **운영자 답변 하나당 1건**입니다(2026-09-15 오너 결정, 그전에는 연속 3건이었습니다 — §0). 회원이 한 번 답장하면 창이 닫히고 "운영자 답변을 기다려 주세요" 안내가 뜨며, 운영자가 한 번 더 답하면 다시 열립니다.

---

## 3. 2026-09-14 — 접수 종류(1:1 문의·버그제보·불법이용제보) 도입 + 관리자 메뉴 "홈페이지 문의"
...
## 4. 2026-09-11 오후 — 고객지원 v2 디자인 적용
...
| 플래그 `NEXT_PUBLIC_FEATURE_MSW_ACCOUNT_FIELDS` | **OFF**              | 꺼져 있으면 계정 ID 프리필이 사실상 비어 있습니다(§11)                       |
...
## 12. 파일 인덱스 (2026-09-10 ~ 09-14 새로 생겼거나 크게 바뀐 것)
...
| `docs/admin/INQUIRY-CHANGELOG.md` · `.html`                                                                                          | (이 문서, §3 접수 종류)                       |
