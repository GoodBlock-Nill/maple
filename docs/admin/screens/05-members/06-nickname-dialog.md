...
| 새 닉네임 | 텍스트 입력 `name="nickname"` | 필수. `.trim()` 후 2~12자(`NICKNAME_MIN_LENGTH`/`NICKNAME_MAX_LENGTH`), 정규식 `^[가-힣a-zA-Z0-9_]+$`. DB: 고유 인덱스 `profiles_nickname_key`(`lower(nickname)`), `profiles.nickname not null` | **현재 닉네임**(`defaultValue`) | hint "2자 이상, 한글·영문·숫자·밑줄. 커뮤니티 목록·랭킹에서는 앞 두 글자만 남기고 가려 보입니다." → `profiles.nickname` |
| 사유 | 텍스트 입력 `name="reason"` | 필수 1자 이상, 최대 200자(`SUSPENSION_REASON_MAX` 재사용 — 정지 사유와 같은 상한) | 빈 칸 | placeholder "예: 부적절한 닉네임 신고 접수", hint "사용자에게는 보이지 않습니다. 감사 로그에만 남는 운영 기록입니다." → **DB 컬럼에 저장되지 않는다.** 감사 로그 `after.reason` 에만 실린다 |
...
| 상황 | 문구 | 표시 위치 |
|---|---|---|
| 형식 위반 | "닉네임은 2자 이상이어야 합니다." / "닉네임은 12자 이하여야 합니다." / "닉네임은 한글·영문·숫자·밑줄만 사용할 수 있습니다." | 입력 옆(`fieldErrors.nickname`) |
