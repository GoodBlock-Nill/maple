...
| 기간 | 라디오 5개 `name="period"` | `z.enum(SUSPENSION_PERIODS)` = `1`·`3`·`7`·`30`·`permanent`. 실패 시 "정지 기간을 선택해 주세요." | **`3`(3일)** — 배열의 두 번째 항목이 `defaultChecked` | 라벨: 1일 / 3일 / 7일 / 30일 / 영구. **임의 날짜 입력은 열지 않는다** — 사람마다 다른 "언제까지"가 나오면 같은 위반에 다른 제재가 나가고 기록을 모아도 기준을 재구성할 수 없다 |
| 사유 | 텍스트 입력 `name="reason"` | 필수 1자 이상("사유를 입력해 주세요."), 최대 200자(`SUSPENSION_REASON_MAX`, "사유는 200자 이하로 입력해 주세요."). `.trim()` 후 판정 | 빈 칸 | placeholder "예: 반복적인 욕설로 신고 3건 누적". hint "사용자 화면의 정지 안내 배너에 “사유: …” 로 그대로 붙습니다. 한 줄로 읽히도록 40자 이내를 권합니다." → `profiles.suspension_reason` |
| 취소 | 버튼(secondary) | 진행 중 비활성 | — | 다이얼로그 닫기(저장 안 함) |
| 정지 | submit(danger) | 진행 중 비활성 | — | `suspendMemberAction` → 성공 시 토스트 "회원을 정지했습니다." + 다이얼로그 닫힘. 진행 중 라벨 "정지 중…" |
...
| 정지 해제 | submit(secondary·sm) | `isSuspended(suspendedUntil)` 일 때만 렌더. 진행 중 비활성 + 라벨 "해제 중…" | `unsuspendMemberAction` → `suspended_until=null`·`suspension_reason=null` → 감사 `member.unsuspend` → `revalidateMember()` |
| 결과 | 토스트 | — | 성공 `{닉네임} 님의 정지를 해제했습니다.`(success) · 실패 `state.formError`(error). 이 버튼만 실패도 **토스트**로 띄운다(다이얼로그가 없어 배너를 걸 자리가 없다) |
...
| `suspended_until <= now()` | 뱃지 "정상" | 기간이 지났다. 값은 남아 있고(프로필 카드 "정지 종료"에 과거 시각이 보인다) 자동으로 지워지지 않는다 |
...
| 파기된 계정 | "개인정보가 파기된 계정입니다. 제재를 적용할 대상이 없습니다." | 다이얼로그 배너 (실제로는 `MemberActions` 가 렌더되지 않아 UI 로는 닿지 않는다 — 직접 POST 방어) |
