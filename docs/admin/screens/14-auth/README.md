...
| 항목 | 값 |
|---|---|
| 경로 | `(auth)/*` 화면 4개 + `auth/callback` 라우트(경로는 아래 상세) |
| 권한 모듈 | 없음(로그인 전). 대신 세 겹의 문(아래 상세) |
| 주요 테이블 | `profiles`·Supabase Auth(`auth.users`)·`admin_invites`(아래 상세) |
| 클라이언트 영향 | 없음(관리자 콘솔 전용). 사용자 사이트와 완전히 별개(아래 상세) |
| 관련 파일 | 페이지·컴포넌트·액션·검증·세션·프록시·환경 경로(아래 상세) |
...
- **권한 모듈** ① `proxy.ts` 낙관적 세션 검사 ② `/auth/callback` 의 `rejectNonAdmin()`·`signInAction` 의 `profiles.role` 확인 ③ 페이지·액션의 `requireAdmin()`/`requirePermission()` + RLS `is_admin()`.
- **주요 테이블** `profiles`(`role` 확인), Supabase Auth(`auth.users`, 세션 쿠키), `admin_invites`(승격 근거 — 트리거가 읽는다).
- **클라이언트 영향** 사용자 사이트에도 같은 이름의 경로가 있지만 완전히 별개이며, 사용자 사이트 `proxy.ts` 는 `/forgot-password`·`/reset-password` 를 `/login` 으로 302 한다.
...
| 코드 | 문구 | 누가 붙이나 |
|---|---|---|
| `not_admin` | 관리자 권한이 없는 계정입니다. 슈퍼어드민에게 관리자 초대를 요청해 주세요. | `/auth/callback` 의 `rejectNonAdmin()`, `requireAdmin()` |
...
- **`not_admin` 문구 상수** `NOT_ADMIN_MESSAGE`.
