...
| 2 | `?token_hash=&type=` (메일 템플릿이 `{{ .TokenHash }}` 를 쓸 때) | `verifyOtp({ type, token_hash })` → `rejectNonAdmin()` → `redirect(next)`. `type` 허용 목록 `ALLOWED_TYPES` = `invite`·`recovery`·`magiclink`·`signup`·`email`(밖이면 `null` 이 되어 3번으로 떨어진다) | 검증 실패·유저 없음 → `/login?error=link_expired` |
| 3 | 둘 다 없음(암시적 흐름, `#access_token=…`) | **해시는 서버로 전송되지 않는다.** `/auth/callback/complete?next=<정규화된 경로>` 로 리다이렉트 — 프래그먼트는 브라우저가 그대로 이어 붙이므로 정보가 유실되지 않는다 | — |

| 파라미터 | 규칙 |
|---|---|
| `next` | `sanitizeNextPath(searchParams.get('next'))` — 내부 절대 경로만, 아니면 `/`. 초대 메일은 `inviteRedirectTo()` 가 `/invite/accept` 로 고정해 보낸다. 재설정 메일은 `/reset-password` |
...
| 요소 | 내용 |
|---|---|
| 화면 | `bg-sidebar` 전체 화면 가운데 카드 하나, 본문은 `aria-live="polite"` 인 문구 `로그인 처리 중입니다…` (입력·버튼 없음) |
...
| 단계 | 내용 |
|---|---|
| 1 | `useRef` 가드 — React 18+ 개발 모드는 이펙트를 두 번 돌리는데 토큰 교환은 한 번만 해야 한다 |
...
| 5 | `establishSessionAction(accessToken, refreshToken)` 서버 액션 호출 → `supabase.auth.setSession()` 이 **httpOnly 쿠키**로 옮긴다(토큰이 자바스크립트가 접근 가능한 저장소에 남지 않는다) |
| 6 | `window.history.replaceState(null, '', pathname)` 로 **주소창의 해시를 지운다** — 남겨 두면 뒤로가기 기록·화면 공유·북마크에 액세스 토큰이 그대로 실린다 |
...
| 규칙 | 내용 |
|---|---|
| 세션 갱신 | 모든 요청에서 `updateSession(request)` 로 Supabase 액세스 토큰을 갱신한다. 리다이렉트를 만들 때는 `redirectWithSession()` 으로 갱신된 쿠키를 반드시 옮겨 싣는다 — 버리면 리프레시된 토큰이 저장되지 않아 임의로 로그아웃된다 |
...
| 비활동 만료 | `admin_last_seen` 쿠키(httpOnly · `sameSite:'lax'` · `secure`(https 일 때) · `path:'/'` · `maxAge` 1800초)에 매 요청 시각을 적는다. `now - stamp > 30분`(`INACTIVITY_LIMIT_MS`)이면 `signOut()` 후 `/login?error=expired` + 쿠키 삭제. 값이 없거나 숫자가 아니면 만료로 보지 않는다(로그인 직후 첫 요청이 튕기지 않게) |
