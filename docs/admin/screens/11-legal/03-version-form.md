...
**목적** 개정본 하나를 작성해 임시저장하거나, 발행하거나, 시행일을 정해 예약한다. 새 초안과 기존 초안 수정이 한 컴포넌트(`LegalForm`)이며 숨은 `id` 하나로 갈린다. 폼은 편집 탭(`?tab=edit`)에만 있고, 발행본을 열면 **에디터까지 통째로** 잠긴다.
...
| 변경 요약 | textarea(2줄) | 선택 ≤200자, 발행본이면 `disabled` | `target.summary` | `summary` |
| 본문 | 리치 에디터 | 필수, 발행본이면 읽기 전용 | `target.contentHtml` | `content_html` |
...
- **본문(잠금)** 발행본에서는 Tiptap 이 `editable: false` 이고 툴바를 그리지 않으며 바탕이 회색(`bg-page`), `aria-readonly="true"` 다.
- **본문(잠금)** 힌트도 갈린다: "발행본이라 읽기 전용입니다. 새 초안을 만들면 이 본문을 그대로 이어받습니다."
- **본문(잠금)** 화면만 회색으로 칠하고 입력을 살려 두면, 운영자가 한참 고친 뒤 저장에서야 거절을 만난다.
...
`<fieldset disabled={isReadOnly}>` 라 발행본을 열면 아래 세 필드가 통째로 잠기고, 저장 버튼은 아예 렌더하지 않는다.

발행 상태 선택(`mode`)은 폼(`LegalForm`)이 들고 있다 — 저장 확인창을 띄울지가 이 값으로 갈리므로, 두 곳이 각자 기억하면 확인 문구와 실제 저장이 어긋난다.
...
| 저장 | 제출 버튼 | 잠금 시 렌더 안 함 | — | `saveLegalVersionAction` |
| 목록으로 | 버튼(링크) | — | — | `/legal` |
...
- **저장** `isPending` 이면 비활성이고 진행 중에는 `저장 중…` 이다. 발행본에서는 버튼 자체가 없다.
- **잠금 안내** 카드 아래 회색 한 줄 대신 폼 맨 위 배너가 안내한다(→ [02-document.md](02-document.md) §1.4).
- **묶음 이유** 이 문서에서 "예약"은 별도 시각 입력이 아니라 "시행일이 아직 오지 않은 발행본"이다. 상태와 시행일을 떨어뜨려 놓으면 예약을 고르고도 시행일을 오늘로 둬서 즉시 공개되는 사고가 난다.

## 1.3 저장 확인창 (`LegalPublishConfirm`)

발행·예약으로 저장할 때만 뜬다. 문구는 `legalConfirmCopy(mode, effectiveDate)`(`admin/lib/validation/legal-state.ts`)가 만든다.

| 상태 | 제목 | 설명 | 실행 버튼 |
|---|---|---|---|
| 발행 | `지금 발행할까요?` | `저장 즉시 사용자 사이트에 공개됩니다. 발행한 개정본은 수정할 수 없습니다.` | `발행` |
| 예약 | `예약 발행할까요?` | `{시행일}부터 사용자 사이트에 공개됩니다. 발행한 개정본은 수정할 수 없습니다.` | `예약` |
| 임시저장 | — | — | 확인 없이 바로 저장 |

- **시행일 표기** `formatEffectiveDate()` 라 `2026년 10월 1일` 형식이다. 제출 순간의 입력값을 읽는다.
- **취소 버튼** 항상 `취소` 이고, Escape·백드롭 클릭으로도 닫힌다(`Dialog`).
- **동작** `onSubmit` 에서 `preventDefault()` 로 서버 액션을 멈추고 확인창을 연다. React 19 는 기본 동작이 막힌 제출에서 form action 을 실행하지 않는다.
- **동작** 확인하면 같은 폼을 `requestSubmit()` 으로 다시 제출한다(그때는 확인창을 건너뛴다).
- **임시저장을 묻지 않는 이유** 되돌릴 수 있는 저장까지 확인창을 띄우면 읽지 않고 누르는 습관이 들어, 정작 발행 확인이 무의미해진다.

## 1.4 저장하지 않은 편집 경고 (`use-unsaved-guard.ts`)

편집 가능한 폼에서 본문(에디터 `onUpdate`)·변경 요약·버전·시행일·발행 상태를 건드리면 `beforeunload` 를 건다.

| 조건 | 결과 |
|---|---|
| 편집했고 저장 중이 아님 | 새로고침·탭 닫기·외부 링크에서 브라우저가 되묻는다 |
| 저장 진행 중(`isPending`) | 경고 없음 — 저장 후 리다이렉트가 이어진다 |
| 발행본(읽기 전용) | 경고 없음 — 고칠 것이 없다 |

- **범위** `<Link>` 로 화면을 옮기는 것은 막지 않는다. 라우터까지 가로채면 저장 후 리다이렉트도 함께 막힌다.
- **문구** 지정하지 않는다. 최신 브라우저는 커스텀 메시지를 무시하고 자체 문구만 보여 준다.

## 1.5 서버 동작 (`saveLegalVersionAction`)
...
## 1.6 임시저장 삭제 (`deleteLegalDraftAction`)
...
- 발행본 수정 차단은 화면(에디터 잠금 + `fieldset disabled` + 저장 버튼 제거)과 액션 양쪽에서 걸린다. 화면만 막으면 직접 POST 로 통과한다.
- 저장 성공 시 토스트가 뜨지 않는다. `redirect` 가 흐름을 끊어 `state.message` 가 화면에 남지 않으며, 이동한 URL 의 `?version=` 이 방금 저장한 개정본을 가리키는 것이 성공 신호다.
- 저장에 성공하면 편집 탭(`?tab=` 없음)으로 돌아온다. 발행했다면 그 화면은 이미 읽기 전용이다.
