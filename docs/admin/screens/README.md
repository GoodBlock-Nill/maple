...
| 07 | 고객지원 — 문의·카테고리·템플릿·FAQ | [07-support/](07-support/README.md) | `/inquiries`(홈페이지·이메일), `/inquiries/[id]`, `/inquiries/categories`, `/inquiries/reply-templates`, `/faqs` | 1:1 문의·버그제보·불법이용제보 접수 → 배정·잠금·답변 → `/support/inquiries/[id]`; 카테고리 → `inquiry-categories` → 접수 폼; FAQ → `faqs` |
