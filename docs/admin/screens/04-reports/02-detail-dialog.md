...
| 제목 줄 | 굵은 텍스트 | 게시글이면 `target.excerpt`(=제목), 댓글이면 `게시글: {원 글 제목}` | 게시글은 제목이 판단의 절반이고, 댓글은 어느 글에 달렸는지가 맥락이다. 원 글이 사라졌으면 `게시글: (삭제됨)` |
| 본문 전문 | `whitespace-pre-wrap`, `max-h-40` 스크롤 | `target.content` | 게시글은 **저장된 HTML 원문 그대로**(렌더하지 않는다 — 태그가 그대로 보인다), 댓글은 평문. 대상이 없으면 `대상을 찾을 수 없습니다(이미 완전히 삭제되었을 수 있습니다).` |
| 원문 보기 | 링크 버튼(ghost sm, 새 탭) | `previewHref` | 목록과 같은 규칙 — 대상이 없거나 **삭제된 대상이면 버튼 자체가 없다** → [01-list.md §원문 링크 규칙](01-list.md#13-표) |
...
| 조치: 대상 숨김 | 라디오 `name="action" value="hide"` | `z.enum(REPORT_ACTIONS)`, 실패 문구 "조치를 선택해 주세요." | **기본 선택** | 설명 "대상을 숨깁니다. 작성자에게도 보이지 않습니다." → `moderateTarget(table,id,'hide')` → `is_hidden = true` |
| 조치: 대상 삭제 | 라디오 `value="delete"` | 〃 | — | 설명 "대상에 삭제 표시를 남깁니다. 복구할 수 있습니다." → `deleted_at = now()` |
| 조치: 작성자 정지 | 라디오 `value="suspend"` | 〃 | — | 설명 "작성자의 글·댓글·신고·좋아요 작성을 기간 동안 막습니다." 고르면 아래 "정지 설정" 묶음이 나타난다 |
| 조치: 조치 없이 처리 | 라디오 `value="none"` | 〃 | — | 설명 "기록만 남기고 대상은 그대로 둡니다." 조치 단계를 건너뛴다 |
| 정지 기간 | 라디오 `name="period"` 5개 (`1`·`3`·`7`·`30`·`permanent` / 라벨 1일·3일·7일·30일·영구) | `action === 'suspend'` 일 때 필수(`superRefine`) — 없으면 "정지 기간을 선택해 주세요." | **`3일`**(`defaultChecked={index === 1}`) | `suspensionUntil()` → `now + N일`(영구는 `9999-12-31T00:00:00.000Z`). **"지금부터 N일"** 이다(자정 기준으로 끊으면 23:59 에 받은 1일 정지가 1분 만에 풀린다) |
| 정지 사유 | `Input` `name="suspensionReason"` | `action === 'suspend'` 일 때 필수 — 비면 "정지 사유를 입력해 주세요." 최대 `SUSPENSION_REASON_MAX = 200`자 | 빈 값, placeholder "예: 반복적인 욕설" | **사용자 화면에 그대로 노출된다.** 힌트: "사용자 화면의 정지 안내 배너에 “사유: …” 로 그대로 붙습니다. 40자 이내를 권합니다." → `profiles.suspension_reason` |
| 처리 메모 | `Textarea` `name="note"` rows=3 | 선택. 최대 `MODERATION_NOTE_MAX = 500`자("메모는 500자 이하로 입력해 주세요."). 빈 문자열은 `null` 로 변환. DB CHECK `reports_note_length (<= 500)` | 빈 값, placeholder "판단 근거를 남겨 주세요." | `reports.note` + `audit_logs.after.note` 양쪽에 저장된다. **화면 힌트는 "감사 로그에만 저장됩니다(신고 테이블에 메모 컬럼이 없습니다)" 라고 적혀 있으나, `20260908002100` 이 `note` 컬럼을 추가한 뒤로는 신고 행에도 저장된다**(힌트·모듈 주석이 코드보다 뒤처져 있다) |
| 함께 처리 | 체크박스 `name="applyToTarget" value="1"` | `z.enum(['0','1'])`. 체크 안 하면 액션이 `'0'` 으로 읽는다 | **`openCountForTarget > 1` 일 때만 렌더되고 기본 체크됨** | 라벨 `같은 대상의 미처리 신고 {n}건을 함께 처리`. 켜면 그 대상의 **`status='open'` 신고 전부**(현재 건 포함)가 한 번에 종결된다 |
| 닫기 | 버튼(secondary) | — | — | 다이얼로그를 닫는다(제출하지 않는다) |
| 처리 완료 | submit 버튼 | — | — | `resolveReportAction`. 처리 중 라벨 `완료 처리 중…` + `disabled`. 성공 시 토스트 `"신고 {n}건을 처리했습니다."` + 다이얼로그 닫힘 |
...
| 폼 배너 | `FormBanner` | — | 숨김 | `state.formError` |
| 기각 사유 | `Textarea` `name="note"` rows=3 `required` | **필수**. `trim().max(500)` 후 `min(1, '사유를 입력해 주세요.')` — 근거 없이 닫힌 신고는 같은 대상이 다시 올라왔을 때 "지난번에 왜 넘겼는지"를 재구성할 수 없다 | 빈 값, placeholder "예: 신고 사유에 해당하지 않는 정상 게시물" | 힌트 "신고자·작성자에게는 보이지 않습니다. 감사 로그에만 남습니다." → `reports.note` + `audit_logs.after.note` |
...
|---|---|---|---|---|---|
| 처리(대상 숨김) | `resolveReportAction` → `moderateTarget('posts'\|'comments', id, 'hide')` | `resolveReportSchema` | `posts`/`comments`.`is_hidden = true`, 그리고 `reports` `status='resolved'`·`note`·`resolved_by`·`resolved_at=now()` | `community.{post\|comment}.hide` + `report.resolve` (**2행**) | 태그 `community-list` |
