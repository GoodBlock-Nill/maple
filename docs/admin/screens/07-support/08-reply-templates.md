...
**동작 상세**
- **설명** — "홈페이지 문의 답변에 불러다 쓰는 상용구입니다. 공통 템플릿은 모든 문의에서, 카테고리 템플릿은 그 분류의 문의에서만 보입니다. 자리표시자({{닉네임}} 등)는 불러오는 순간 그 문의의 정보로 바뀝니다."

## 2. 묶음 카드 (`InquiryReplyTemplateGroup`)

| 필드/컨트롤 | 종류 | 필수·제한(검증) | 기본값 | 동작 / 상호작용 |
|---|---|---|---|---|
| 묶음 제목 | 텍스트 | — | 묶음별 제목 형식(아래) | 라벨만 적지 않는 이유(아래) |
| 숨긴 카테고리 뱃지 | `Badge neutral` | 그 카테고리가 비활성일 때만 | — | "왜 답변 화면에 안 보이나"를 설명한다 |
| 순서 저장 | 버튼(secondary, sm) + 폼 | write **AND 항목이 2개 이상**일 때만 렌더. 순서가 바뀐 경우에만 활성 | "순서 저장" / "저장 중…" | `reorderInquiryReplyTemplatesAction` 실행(아래) |
| 템플릿 추가 | 버튼(secondary, sm) | write | **그 묶음의 `categoryId`** 가 기본 선택 | 다이얼로그 |
| 빈 묶음 | 안내문 | — | — | "등록된 템플릿이 없습니다. 이 분류의 문의에서는 공통 템플릿만 보입니다." |

**동작 상세**
- **묶음 제목(기본값)** — 공통 묶음은 `공통(모든 카테고리) ({n})`, 나머지는 `{종류} · {라벨} ({n})`(예: `버그제보 · 접속·서버 (3)`).
- **묶음 제목(동작)** — 라벨만 적으면 세 창구의 분류가 섞인 화면에서 "이 문안이 어디에서 보이는가"를 알 수 없다.
- **순서 저장** — hidden `ids`(콤마 연결). `reorderInquiryReplyTemplatesAction` → 그 묶음 안 `sort_order` 를 0..n-1 로 다시 쓴다 → 감사 `inquiry_reply_template.reorder`(`after: { ids }`) → 토스트 "순서를 저장했습니다."
...
| 끄기·켜기 | 버튼(ghost, sm) + 폼 | 필드·액션 계약(아래) |
| 수정 | 버튼(secondary, sm) | 다이얼로그(프리필된 값) |
| 삭제 | 버튼(danger, sm) | 확인 다이얼로그(§3.1) |

**동작 상세**
- **끄기·켜기** — hidden `templateId`, `isActive`(반대값). `toggleInquiryReplyTemplateAction` → `is_active` + `updated_by` → 감사 `inquiry_reply_template.update` → 토스트 "템플릿을 켰습니다." / "템플릿을 껐습니다."
...
| 수정 | `updateInquiryReplyTemplateAction` | 동일 | update, 카테고리 이동 시 순번 재계산(아래) | `.update`(before/after) | 동일 |
...
**동작 상세**
- **수정(DB)** — update. **카테고리를 옮기면 새 묶음의 맨 뒤로 `sort_order` 를 다시 잡는다**(그대로 두면 순번이 겹쳐 화면 순서가 갈린다).
...
- 마이그레이션 `20260914000400` §8 이 "추가 정보 요청" · "제보 증거 자료 요청" · "오류 재현 정보 요청" · "서버 점검 안내" 시드 템플릿의 꼬리 문구를 **"이 문의에 답장으로 남겨 주세요. 문의가 처리 중 상태인 동안 답장할 수 있습니다."** 로 되돌렸다(20260914000300 의 "새 문의로 접수해 주세요"를 뒤집은 것). 완료 계열(처리 완료 안내 · 아이템 지급 처리 완료 · 데이터 복구 안내)은 그대로 둔다 — 그 답변은 상태를 '답변 완료'로 닫으면서 나가 답장 창이 없다. **회원 답장 UI 는 2026-09-14 부터 배포됐다**(관리자 커밋 `df57060` · 사용자 커밋 `6edbcf9`, README §2.7) — 이 문구가 가리키는 답장 칸이 실제로 `/support/inquiries/[id]` 상세에 뜬다(처리 중 · 운영자 답변 후 · 그 답변에 아직 답장하지 않았을 때 — 운영자 답변 하나당 답장 1건, 2026-09-15).
...
| 이름 중복 (23505) | 필드 오류 `name`: "같은 카테고리에 같은 이름의 템플릿이 있습니다."(아래) |
| 고른 카테고리가 그 사이 삭제됨 (23503) | 필드 오류 `categoryId`(아래) |
| 대상 행 없음 | "템플릿을 찾을 수 없습니다." |
| 순서 저장 일부 실패 | "순서를 저장하지 못했습니다…"(아래) |
| 정렬 페이로드가 깨짐 | "정렬 정보를 읽지 못했습니다." |
| 그 밖 저장·삭제 실패 | 등록/수정/삭제별 문구(아래) |
| 읽기 전용 관리자 | ▲▼·순서 저장·추가·토글·수정·삭제가 렌더되지 않는다. 등록 버튼도 헤더에서 빠진다 |

- **이름 중복(23505)** — 유니크 인덱스가 `coalesce(category_id, '00000000-…')` + `name` 이라 **공통끼리도** 이름이 겹칠 수 없다.
- **고른 카테고리가 그 사이 삭제됨(23503)** — "고른 카테고리가 사라졌습니다. 목록을 새로고침한 뒤 다시 시도해 주세요."
