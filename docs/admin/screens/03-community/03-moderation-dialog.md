...
|---|---|---|---|---|
| 선택 개수 안내 | 텍스트(muted) | — | `{라벨}을 선택해 일괄 숨김할 수 있습니다.` | 0건이면 안내 문구, 1건 이상이면 `{n}건 선택됨`. 조사는 `josa()` 가 붙인다(게시글**을** / 댓글**을**) |
| 전체 선택/해제 | 버튼(ghost sm) | — | — | `document.querySelectorAll('input[form="bulk-hide-form"]:not(:disabled)')` 를 모두 켜거나 끈다. **현재 페이지 20건 범위**이고, 이미 숨김·삭제된 행(=`disabled`)은 건드리지 않는다 |
...
| 항목 | 값 |
|---|---|
| 권한 | `requireAnyPermission(['community', 'reports'], 'write')` — 신고를 종결하려면 대상 글을 숨겨야 하는데, 그때마다 커뮤니티 쓰기까지 요구하면 '신고 담당' 역할이 성립하지 않는다 |
| 행위자 | **인자로 받지 않는다.** `'use server'` 모듈의 export 는 전부 액션 엔드포인트로 열리므로, 행위자를 받으면 직접 POST 로 남의 이름을 적어 넣을 수 있다. 인가와 신원은 함수 안에서 다시 확인한다 |
...
| 상황 | 결과 |
|---|---|
| `id` 가 uuid 가 아님 | `fieldErrors.id` "대상을 찾을 수 없습니다." (다이얼로그·바에는 이 필드를 그리는 자리가 없어 화면에는 아무 문구도 뜨지 않는다) |
| `on` 이 `'0'`/`'1'` 이 아님 | `fieldErrors.on` (zod 기본 문구) |
| 선택 0건으로 일괄 제출 | `formError` "대상을 한 건 이상 선택해 주세요." (버튼이 `disabled` 라 화면에서는 닿기 어렵다) |
| 101건 이상 선택 | `formError` "한 번에 100건까지 처리할 수 있습니다." — 한 페이지가 20건이라 화면에서는 닿지 않는다(직접 POST 방어) |
| DB 오류 | 위 표의 고정 문구 + `console.error('[community] …', 원문)`. Postgres 원문은 화면에 노출하지 않는다(제약명·정책명이 새어 나간다) |
| 권한 없음 | `/?error=forbidden` 리다이렉트 |
| 이미 삭제된 행에 삭제를 다시 검 | 막지 않는다. `deleted_at` 이 새 시각으로 덮인다(다이얼로그에서는 이미 "복구"로 바뀌어 있어 화면에서는 닿지 않는다) |
