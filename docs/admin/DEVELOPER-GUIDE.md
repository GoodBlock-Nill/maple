...
| `subtypes`    | **세부 문의 유형**(text[] · ≤ 20개 · 각 ≤ 30자). 사용자 폼의 유형 셀렉트가 되고, 고른 값이 `inquiries.type` 에 저장된다. |
...
- **세부 문의 유형(2026-09-11).** 유형 셀렉트는 더 이상 고정 3종('문의 · 신고 · 제안')이 아니라 **고른 카테고리의
  `subtypes`** 다(마이그레이션 `20260910000700`). 규칙은 넷이다.
  - 카테고리를 바꾸면 유형 선택은 **비워진다** — 남겨 두면 "접속·서버 + 콘텐츠 개선 의견" 같은 조합이 저장된다.
  - `subtypes` 가 빈 카테고리('기타·건의' 시드)는 셀렉트를 **잠그고** `기타` 를 hidden 으로 실어 접수한다
    (`lib/utils/inquiry-subtypes.ts` `INQUIRY_SUBTYPE_FALLBACK`). 셀렉트를 지우지 않는 이유는 두 칸 격자가 흔들리지 않게 하려는 것.
  - 서버는 카테고리와 유형을 **함께** 본다(`createInquirySchema` 의 `checkSubtype`). 수정 화면은 그 문의가 접수 당시
    들고 있던 유형(옛 '문의' 등)을 하나 더 허용한다 — 막으면 제목만 고치려던 사용자가 유형부터 다시 정해야 한다.
  - 프리필 양식에서는 "세부 문의 유형" 안내 블록을 걷어냈다(`20260910000700` = 원본 시드와 같은 행,
    `20260910000800` = 운영자가 손댄 행에서 **그 블록만** 정규식으로 제거). 두 곳에 두면 사용자가 같은 것을 두 번 고른다.
...
- **필터 옵션.** 목록 필터의 카테고리는 DB 의 카테고리(비활성 포함) + **데이터에만 남은 옛 라벨**이다
  ...
  **유형 필터**도 같은 규격이다(`public.inquiry_type_usage()` · `getInquiryTypeFilterOptions()`): 카테고리를 고른
  뒤 '검색' 을 누르면 그 카테고리의 세부 유형만 남고, 어느 경우든 데이터에만 남은 옛 값('문의' · '신고' · '제안' ·
  이메일 문의의 'general')이 뒤에 붙는다. 필터 폼은 자바스크립트 없이 도는 GET 폼이라 왕복이 곧 갱신이다.
...
**1:1 문의 필수 항목(2026-09-11 제품 결정).** 접수의 필수 항목은 **카테고리 · 세부 문의 유형 · 메이플월드 계정 ID ·
제목 · 문의 내용 · 개인정보 수집 동의** 여섯이고, **첨부만 선택**이다. 라벨에는 별표(`*`)가 붙고, 하나라도 비어 있으면
제출 버튼이 잠긴 채 그 아래에 이유("필수 항목(\*)을 모두 입력해 주세요.")가 선다
(`components/support/InquiryForm.tsx` · `isInquiryFormFilled()`). 서버 스키마가 같은 규칙을 다시 본다 — 잠금은 편의이고
신뢰 경계는 서버다.

- **계정 ID 는 새 컬럼이 아니다.** `inquiries.account_id` 가 그 값이다(20260908000400 주석: "MSW 계정 ID"). 같은 뜻의
  컬럼을 하나 더 두면 화면·검색·통계마다 "어느 쪽이 진짜인가"를 다시 정해야 한다. 바뀐 것은 셋이다 —
  필수화, 서식 완화(숫자 10~~20자리 → 영문·숫자·`_`·`-` 2~~40자), 길이 CHECK(`inquiries_account_id_length` ≤ 40,
  마이그레이션 `20260910000900`). 서식을 푼 이유는 클라이언트가 보여 주는 ID 모양이 바뀌었을 때 **접수 자체가 막히는**
  것이 오탈자보다 비싸기 때문이다. 자릿수 확인은 운영자가 화면에서 한다.
- **컬럼은 계속 nullable 이다.** 이메일로 들어온 문의는 계정 ID 를 알 방법이 없고, 필수가 되기 전의 옛 문의에도 값이 없다.
  필수는 웹 폼의 규칙이라 그 경계(서버 액션)에서만 강제한다.
- **화면.** 프로필에 월드 UID(`profiles.msw_uid`)가 연동돼 있으면 접수 폼에 미리 채워진다(수정 가능). 표시는
  사용자 상세·관리자 목록/상세 모두 **마스킹**(`1234****000`)이고, 관리자 검색은 계정 ID 를 그대로 훑는다(`account_id.ilike`).

**1:1 문의 첨부 — 이미지·PDF 와 영상이 다른 길로, 이제 개수도 다른 자리로 온다(2026-09-11 오너
지시).** 이미지·PDF 는 3개, 영상은 2개 — 각자 따로 상한을 갖고, 합쳐서 최대 **5개**다. DB
CHECK 셋이 이 규칙을 나눠 지킨다 — `inquiries_attachments_file_kind_max_3`(영상이 아닌 것
≤3) · `inquiries_attachments_video_kind_max_2`(영상 ≤2) · `inquiries_attachments_max_5`
(합계 ≤5, 마이그레이션 `20260911000600`). 종류는 jsonb 원소의 `mimeType` 이 `video/` 로
시작하는지로 가른다(별도 열은 없다).

| 종류       | 형식                         | 크기                   | 개수                   | 전송 경로                      |
| ---------- | ---------------------------- | ---------------------- | ---------------------- | ------------------------------ |
| 이미지·PDF | jpg · png · gif · webp · pdf | 각 5MB · **합계 12MB** | **3개**(영상과 별도)   | 폼 → 서버 액션 본문 → 스토리지 |
| 영상       | mp4 · mov · webm · m4v       | 각 **100MB**           | **2개**(이미지와 별도) | 브라우저 → 스토리지 **직접**   |
