...
> - 로그인 상태: ~~`닉네임님` + 로그아웃(흰 버튼)~~ → **재변경(제품 결정
>   2026-09-08 후속)**. 로그아웃 버튼을 헤더에 상시 노출하지 않는다. 대신
>   **닉네임 트리거 하나**(16px semibold `#2a2a2a` + 28px 아바타 원 — `avatar_url`
>   이 있으면 이미지, 없으면 첫 글자 + `bg-ink/10` — + 12px 캐럿)를 두고, 클릭하면
>   헤더 우측 아래 정렬로 드롭다운(흰 배경 · radius 12px · `border-line-soft` 1px ·
>   `shadow-menu` · padding 8px · 항목 h 40px)이 열려 **"내 정보"**(`/account`)와
>   **"로그아웃"**을 보여준다. 컴포넌트: `components/layout/UserMenu.tsx`
>   (`components/board/LinkMenu.tsx` 의 열기/닫기/포커스 패턴을 참고해 새로
>   구현 — 로그아웃이 링크가 아니라 폼(POST) 이어야 해서 그대로 재사용하지
>   않았다). 헤더 치수(높이 70·우측 그룹 위치)는 그대로다.
> - 모바일(<1024px): 로고 + **로그인**(미로그인일 때만) + 햄버거. 드로어는
>   로그인 상태일 때 상단에 닉네임(+아바타) 블록과 "내 정보"/"로그아웃" 행을
>   두고, 하단의 별도 로그아웃 버튼은 없앴다(중복 노출 금지).
> - 나머지 치수(메뉴 간격 50px, 우측 그룹 간격 12px, 로고 99×36, 라운드 10px)는 그대로다.
> - `/register` 는 화면을 없애고 `/login` 으로 **308 영구 이동**한다(옛 링크 보존).
> - **내 정보** (`/account`, 신규): 로그인 카드와 같은 셸(`AuthShell`/`AuthCard`).
>   편집 가능: 닉네임, 메이플스토리 월드 계정 UID(숫자 10~20자), 프로필 코드
>   (`#`+영문소문자·숫자 4~10자) — 온보딩에서도 함께 받는다(둘 다 필수).
>   읽기 전용: 로그인 방식(제공자 아이콘 + 라벨, `social-icons.tsx`), 가입일,
>   약관·개인정보처리방침 동의 일시. 하단에 보조(흰 배경) 로그아웃 버튼.
