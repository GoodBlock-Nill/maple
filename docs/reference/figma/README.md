...
## 마이페이지 v2 (같은 파일 2UmKcpmy55IqMZ7Sg6vTeW, 2026-09-11)

쿠폰·문의내역 탭 제거, 계정 관리 축소(닉네임 + 이메일), **계정 연동** 탭 추가, 헤더 로그인 상태 드롭다운,
푸터 마이페이지 변형(패널 1200@120 · 연락처 텍스트). 수치는 `mypage-v2-spec.md`.

| 화면 | 노드 ID | 크기 | 스크린샷 |
|---|---|---|---|
| 마이페이지\_프로필(계정 관리) | `166:13113` | 1440×1969 | mypage-v2/profile-pc.png |
| 마이페이지\_쿠폰(이름만 — 실제는 계정 연동) | `166:13195` | 1440×1971 | mypage-v2/link-pc.png |
| 모바일 계정 관리 | `166:13436` | 375×1685 | mypage-v2/profile-mobile.png |
| 모바일 계정 연동 | `166:13299` | 375×1641 | mypage-v2/link-mobile.png |
| 헤더(로그인 상태 + 드롭다운) | `166:13194` | — | profile-pc.png 상단 |
| 푸터(마이페이지 변형) | `178:20156` | 1200×353 패널 | link-pc.png 하단 |

아이콘 `public/images/mypage-v2/{tab-account,tab-link,check-on,arrow,chevron-down,google}.svg`.
배경·푸터 그림·여우 GIF 는 v1 자산(`public/images/mypage/`)을 그대로 쓴다.

> ~~`mypage-spec.md`(2041:2958 · 2041:3128 · 2041:3237, `mypage/*.png`)~~ 는 **대체된 시안**이다
> (2026-09-11). 프로필·비밀번호·아바타 카드와 쿠폰·문의내역 탭은 구현에서 빠졌고, 두 경로
> (`/account/coupon` · `/account/inquiries`)는 프록시가 `/account` 로 돌려보낸다. 결정 이력으로 문서는 남긴다.

## 고객지원 v2 (같은 파일 2UmKcpmy55IqMZ7Sg6vTeW, 2026-09-11)

카드 안쪽(좌측 메뉴 · 내 문의 내역 목록 · 문의 상세 · 수정 폼)과 헤더 바 폭·활성색, 푸터
패널을 다시 그렸다. **로직·데이터 흐름은 그대로**(카테고리/세부 유형/계정 ID/첨부 3+2/동의/
프리필/접수번호/수정·취소 권한). 수치는 `support-v2-spec.md`.

| 화면 | 노드 ID | 크기 | 스크린샷 |
|---|---|---|---|
| 내 문의 내역 목록(페이지네이션) | `166:13584` | 1440×1792 | support-v2/pc-1.png |
| 문의 상세 — 접수대기(수정·취소, 답변 없음) | `166:14869` | 1440×1792 | support-v2/pc-2.png |
| 문의 상세 — 답변 완료 | `166:15002` | 1440×1792 | support-v2/pc-3.png |
| 문의 수정 폼 | `166:15141` | 1440×1792 | support-v2/pc-4.png |
| 모바일 목록 · 상세(접수대기) · 상세(답변 완료) · 수정 폼 | `166:13911` · `166:14121` · `166:14296` · `166:14655` | 375×{1680,1629,1775,1976} | support-v2/m-1~4.png |
| 헤더 바(전 페이지 공통, 폭 1200 + 핑크 활성 밑줄) | `166:13606` | 1200×66 | pc-1.png 상단 |

아이콘 `public/images/support/v2/*.svg`(신규), 좌측 메뉴 아이콘은 기존
`public/images/support/icon-*.svg` 그대로. 구현 상세는 `docs/admin/INQUIRY-GUIDE.md` §2.7.

## 뉴스목록 v2 (같은 파일 2UmKcpmy55IqMZ7Sg6vTeW `229:4490` "뉴스목록_시안_2", 2026-09-11)

툴바에 **보기 전환 버튼**(카드형/가로형, `?view=card|list`, 기본값 card)이 추가됐다. 카테고리
칩·검색은 현행 그대로. 수치는 `news-v2-spec.md`.

| 요소 | 노드 ID | 스크린샷 |
|---|---|---|
| 뉴스목록 카드형 전체 | `229:4490` | news-v2/list-pc.png |

아이콘 `public/images/news/v2/{pin.png, view-card.svg, clock-12.svg, eye-15.svg}`. 상단
밴드·마스코트·제목·푸터는 현행 news 변형 그대로. 헤더는 고객지원 v2 와 같은 1200px 바(별도
작업으로 전 페이지 공통 반영).

## 모바일 내비게이션(햄버거 드로어) v2 (같은 파일 2UmKcpmy55IqMZ7Sg6vTeW, 2026-09-11)

우측 슬라이드 드로어를 다시 그렸다(폭 290, 행 48 · 좌우 여백만, 회색 카드 폐기). 로그인 상태에
따라 계정 행 펼침/접힘이 달라진다. 수치는 `mobile-nav-v2-spec.md`.

| 화면 | 노드 ID | 스크린샷 |
|---|---|---|
| 비로그인(상단 "로그인" 알약) | `16:3433` | mobile-nav-v2/drawer-guest.png |
| 로그인 — 계정 줄 접힘 | `16:3531` | drawer-user-closed.png |
| 로그인 — 계정 줄 펼침(내 정보/로그아웃) | `13:2883` | drawer-user-open.png |

GNB 5행(뉴스·커뮤니티·가이드·랭킹·고객지원, 소개는 플래그로 숨김), 하단 고정 블록(메이플
월드·디스코드 바로가기)은 v1 의 버튼 2개 대신 텍스트 + 외부 링크 아이콘으로 바뀌었다.
아이콘은 시안 SVG 내보내기가 불완전해 인라인 SVG 로 재현했다.

## 헤더·푸터 변형 이력

헤더·푸터는 화면마다 다른 변형을 쓴다(`components/layout/footer-variants.ts` ·
`components/layout/page-hero.ts`가 단일 출처). 마이페이지 v2 가 1200×353 어두운 글래스
패널을 처음 도입했고, 고객지원 v2 가 같은 패널을 재사용했다(§"마이페이지 v2" ·
"고객지원 v2" 참고). 헤더 바는 v1 에서 내용 폭만큼 hug 하던 것을 고객지원 v2 가 전 페이지
공통 1200px + 핑크 활성 밑줄로 통일했다(뉴스목록 v2 도 이 헤더를 그대로 쓴다). 별도의
"footer v3" 시안은 없다 — 지금까지의 변형은 v1(기본) → v2(마이페이지/고객지원 패널) 둘뿐이다.

시안 PNG 실측(1440) — 구현이 맞춰야 하는 기준값:

| 요소 | 값 |
|---|---|
| 제목 블록 y | 334 |
| 사이드바 | x 120 · w 268 · 탭 높이 68(48 아이콘 박스 + 패딩 10) · 탭 간격 10 |
| 카드 | x 420 · y 461 · w 900 · h 269(계정 관리) / 408(계정 연동, 플래그 ON) |
| 입력 | x 444 · w 413 · h 54, 라벨↔입력 10 · 필드 간격 24(계정 관리) / 32(계정 연동) |
| 버튼 | "변경하기" x 868 w 99 h 54 · "계정 연동하기" x 444 w 183 h 54 |
| 마케팅 박스 | y 811.5 · h 56 · w 900 (제목 y 761.6) |
| 구분선 / 회원 탈퇴 | y 899.6 / y 931.6 (트리거 y 960.7) |
| 푸터 패널 | x 120 · y 1546 · 1200×353 (내부 패딩 40/140, 좌측 블록 371) |
| 여우 GIF | x 1080 · y 1600 · 207×167 |
| 헤더 드롭다운 | 189×124 · top 86 · 항목 54(패딩 16) |
| 폰(375) | 제목 y 176 · 카드 343 @ (16,278) · 세그먼트 탭 47 · 입력 319×42 · 버튼 99×48 |
