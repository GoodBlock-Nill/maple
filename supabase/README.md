...
로그인 수단은 **간편로그인뿐**이다. 화면에 있는 버튼은 **구글·네이버 둘**이고
(2026-09-10 시안 auth-v2), 카카오는 시안에서 숨김이라 버튼만 빼 두었다 — 제공자
목록(`SOCIAL_PROVIDERS`)과 스텁·아바타 처리에는 그대로 남아 있어 언제든 되살릴 수 있다.
이메일·비밀번호 가입과 비밀번호 재설정은 제거되었다(`/signup` · `/forgot-password` ·
`/reset-password` 는 모두 `/login` 으로 302).
